...
SET TCL=.\tcl8.6\windows\64
SET SRC=.\source
SET OUT=.\output\windows\64
SET PRJ=kitmom
SET DIRS=%SRC%
...
SET CCFLAGS=-DUSE_TCL_STUBS %INCS% -Wall -Wextra -Os
...
windres --target pe-x86-64 -I"%TCL%\include" --input "%SRC%\%PRJ%.rc" --output "%OUT%\%PRJ%.res" --output-format=coff
...
mkdir ".\lib\%PRJ%"
:30
copy "%OUT%\%PRJ%.dll" ".\lib\%PRJ%\%PRJ%64.dll"
copy ".\%PRJ%\*" ".\lib\%PRJ%"
